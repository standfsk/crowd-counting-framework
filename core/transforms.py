...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, image: Tensor, label: Union[List, Tensor]) -> Tuple[Tensor, Tensor]:
...
class CustomRandomResizeAndCrop(object):
    ...
    def __call__(self, image: Tensor, label: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        if torch.rand(1) < self.p:
            resized_height = int(in_height * scale)
            resized_width = int(in_width * scale)
            if min(resized_width, resized_height) > self.crop_size:
                image, label = _resize(image, label, resized_height, resized_width)

        top = torch.randint(0, image.shape[1] - self.crop_size + 1, (1,)).item()
        left = torch.randint(0, image.shape[2] - self.crop_size + 1, (1,)).item()
        return _crop(image, label, top, left, self.crop_size, self.crop_size)


class CustomCompose:
    def __init__(self, transforms: List[Callable[[Tensor, Tensor], Tuple[Tensor, Tensor]]]) -> None:
        self.transforms = transforms

    def __call__(self, image: Tensor, label: Tensor) -> Tuple[Tensor, Tensor]:
        for transform in self.transforms:
            image, label = transform(image, label)  # Pass both image and label
        return image, label
...
def get_transforms(config: object) -> CustomCompose:
    if config.network in ["dmcount", "fusioncount", "ffnet"]:
        transforms = CustomCompose([
            ...
            RandomApply([
                ColorJitter(brightness=config.brightness,
                            contrast=config.contrast,
                            saturation=config.saturation,
                            hue=config.hue),
                GaussianBlur(kernel_size=config.kernel_size, sigma=(0.1, 5.0)),
                PepperSaltNoise(saltiness=config.saltiness,
                                spiciness=config.spiciness),
            ], p=(config.jitter_prob, config.blur_prob, config.noise_prob)),
        ])
    elif config.network == "clip_ebc":
        if config.sliding_window:
            if config.resize_to_multiple:
                transforms = Resize2Multiple(config.window_size, stride=config.stride)
            elif config.zero_pad_to_multiple:
                transforms = ZeroPad2Multiple(config.window_size, stride=config.stride)
            else:
                transforms = None
        else:
            transforms = CustomCompose([
    ...
    elif config.network == "apgcc":
        transforms = CustomCompose([
            RandomResizedCrop((config.input_size, config.input_size), scale=(config.min_scale, config.max_scale)),
            RandomHorizontalFlip(),
        ])

    elif config.network == "cltr":
        transforms = CustomCompose([
            CustomRandomResizeAndCrop(crop_size=config.crop_size, scale=(config.min_scale, config.max_scale)),
            RandomHorizontalFlip(),
            RandomGrayscale(p=config.gray_scale_prob)
        ])

    elif config.network == "p2pnet":
        transforms = CustomCompose([
            Resize(size=(config.input_size, config.input_size))
        ])

    elif config.network == "steerer":
        transforms = CustomCompose([
            CustomRandomResizeAndCrop(crop_size=config.crop_size, scale=(2.0, 2.0), p=1.0),
            RandomHorizontalFlip()
        ])
