...
class DACELoss(nn.Module):
    def __init__(
        ...
        **kwargs: Any
        ...
        count_loss_type = count_loss_type.lower()
        assert count_loss_type in ["mae", "mse", "dmcount"], f"Expected count_loss to be one of ['mae', 'mse', 'dmcount'], got {count_loss}"
        self.count_loss_type = count_loss_type
        if self.count_loss_type == "mae":
    ...
    def forward(
        ...
        if self.use_dm_loss:
            ...
            loss_info = {
                ...
                f"{self.count_loss_type}_loss": count_loss.detach(),
            }
