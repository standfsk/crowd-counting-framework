...
class AnchorPoints(nn.Module):
    ...
    ) -> np.ndarray:  # shape is feature map shape
...
class Basic_Decoder_Model(nn.Module):
    def __init__(
        ...
        line: int = 2,
        row: int = 2,
        inner_planes: int = 256,
        feat_layers: List[int] = [3, 4],
        ...
        **kwargs: Any
        ...
        self.fpn = FPN(self.in_planes[0], self.in_planes[1], self.in_planes[2], self.in_planes[3], self.inner_planes,
                       self.feat_layers)
        ...
        self.anchor_points = AnchorPoints(pyramid_levels=self.feat_layers[0], stride=anchor_stride, row=row,
                                          line=line)  # get featmap coords offset
...
class IFI_Decoder_Model(nn.Module):
    def __init__(
        ...
        anchor_stride: Optional[int] = None,
        sync_bn: bool = False,
        dilations: Tuple[int, int , int] = (2, 4, 8),
        require_grad: bool = False,
        head_layers: Tuple[int, int, int] = [512, 256, 256],
        out_type: str = 'Normal',
        pos_dim: int = 24,
        ultra_pe: bool = False,
        learn_pe: bool = False,
        ...
        **kwargs: Any
        ...
        if 1 in self.feat_layers:
            self.enc1 = nn.Sequential(nn.Conv2d(self.in_planes[0], inner_planes, kernel_size=1),
                                      norm_layer(inner_planes), nn.ReLU(inplace=True))
        if 2 in self.feat_layers:
            self.enc2 = nn.Sequential(nn.Conv2d(self.in_planes[1], inner_planes, kernel_size=1),
                                      norm_layer(inner_planes), nn.ReLU(inplace=True))
        if 3 in self.feat_layers:
            self.enc3 = nn.Sequential(nn.Conv2d(self.in_planes[2], inner_planes, kernel_size=1),
                                      norm_layer(inner_planes), nn.ReLU(inplace=True))
        if 4 in self.feat_layers:
            if self.no_aspp:
                self.head = nn.Sequential(nn.Conv2d(self.in_planes[-1], inner_planes, kernel_size=1),
                                          norm_layer(inner_planes), nn.ReLU(inplace=True))
            else:
                self.aspp = ASPP(self.in_planes[-1], inner_planes=inner_planes, sync_bn=sync_bn, dilations=dilations)
                self.head = nn.Sequential(
                    nn.Conv2d(self.aspp.get_outplanes(), inner_planes, kernel_size=3, padding=1, dilation=1,
                              bias=False),
        ...
        # Auxiliary Anchors
        self.aux_enabled = kwargs['aux_enabled']
        self.aux_num_layers = kwargs['aux_num_layers']
    ...
    def forward(
        ...
        if not self.aux_enabled or not self.training:
            return out
        else:
