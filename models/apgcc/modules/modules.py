...
class ASPP(nn.Module):
    ...
    def __init__(
        ...
        sync_bn: bool = False,
        bn: bool = False,
        dilations: Tuple[int, int, int] = (12, 24, 36)
    ) -> None:
        ...
        if bn == False:
            self.conv1 = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)),
                                       nn.Conv2d(in_planes, inner_planes, kernel_size=1, padding=0, dilation=1,
                                                 bias=False),
...
class ifi_simfpn(nn.Module):
    def __init__(
        ...
        head_layers: List[int] = [512, 256, 256],
        pos_dim: int = 40,
        ultra_pe: bool = False,
        learn_pe: bool = False,
        unfold: bool = False,
...
def make_coord(
    ...
    ret = torch.stack(torch.meshgrid(*coord_seqs, indexing=None), dim=-1)
    if flatten:
...
def ifi_feat(
    ...
    stride: int = 1,
    local: bool = False
) -> Union[Tuple[Tensor], Tuple[List[Tensor], List[Tensor], List[Tensor]]]:
    '''
    ...
    feat_coords = make_coord((hh, ww), flatten=False).to(res.device).permute(2, 0, 1).unsqueeze(0).expand(res.shape[0], 2,
                                                                                                  *(hh, ww))
