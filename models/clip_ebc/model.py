...
class CLIP_EBC(nn.Module):
    def __init__(
        ...
        vpt_dropout: Optional[float] = None,
        ...
        if backbone in resnet_backbones:
            self.image_encoder = getattr(modules, f"{backbone}_img")(features_only=True, out_indices=(-1,),
                                                                     reduction=reduction)
            ...
            assert vpt_dropout is not None, "Expected vpt_drop to be a float, got None."

            self.image_encoder = getattr(modules, f"{backbone}_img")(features_only=True, input_size=input_size)
            ...
            for idx in range(self.image_encoder_depth if self.deep_vpt else 1):
                ...
                setattr(self, f"vpt_drop_{idx}", nn.Dropout(vpt_dropout) if vpt_dropout > 0 else nn.Identity())
        ...
        if decoder_cfg is not None:
            ...
            self.image_decoder = make_resnet_layers(decoder_block, decoder_cfg, in_channels=self.channels, expansion=1,
                                                    dilation=1)
        ...
        self.text_encoder = getattr(modules, f"{backbone}_txt")()
    ...
    def _tokenize_text_prompts(self) -> None:
        self.text_prompts = modules.tokenize(self.text_prompts)
    ...
    def _forward_vpt(self, x: Tensor) -> Tuple[Tensor]:
        ...
        num_h_patches, num_w_patches = height // self.image_encoder.patch_size[0], width // \
        ...
        image_features = torch.cat([
            self.image_encoder.class_embedding + torch.zeros(batch_size, 1, image_features.shape[-1],
        ...
        assert image_features.shape[0] == num_h_patches * num_w_patches + 1 and image_features.shape[
            1] == batch_size, f"Expected image_features to have shape [num_patches + 1, B, C], got {image_features.shape}."
...
def _clip_ebc(
    ...
    return CLIP_EBC(
        backbone=backbone,
        bins=config.bins,
        anchor_points=config.anchor_points,
        reduction=config.reduction,
        freeze_text_encoder=freeze_text_encoder,
        prompt_type=config.prompt_type,
        input_size=config.input_size,
        num_vpt=config.num_vpt,
        deep_vpt=not config.shallow_vpt,
        vpt_dropout=config.vpt_dropout,
