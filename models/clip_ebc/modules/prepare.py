...
def prepare(model_name: str) -> None:
    ...
    model = load(model_name_map[model_name], device=device).to(device)
    image_encoder = model.visual.to(device)
    text_encoder = CLIPTextEncoderTemp(model).to(device)
    torch.save(model.state_dict(), os.path.join(weight_dir, f"clip_{model_name}.pth"))
    torch.save(image_encoder.state_dict(), os.path.join(weight_dir, f"clip_image_encoder_{model_name}.pth"))
    torch.save(text_encoder.state_dict(), os.path.join(weight_dir, f"clip_text_encoder_{model_name}.pth"))
    model_config = {
    ...
    with open(os.path.join(config_dir, f"clip_{model_name}.json"), "w") as f:
        json.dump(model_config, f, indent=4)
    with open(os.path.join(config_dir, f"clip_image_encoder_{model_name}.json"), "w") as f:
        json.dump(image_encoder_config, f, indent=4)
    with open(os.path.join(config_dir, f"clip_text_encoder_{model_name}.json"), "w") as f:
