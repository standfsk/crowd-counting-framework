...
def run(local_rank: int, nprocs: int, config: object) -> None:
    ...
    for epoch in range(start_epoch, config.epochs + 1):  # start from 1
        ...
        model, optimizer, loss_info = train(model, train_loader, loss_fn, optimizer, grad_scaler, device,
                                            local_rank, nprocs, config.num_queries,
                                            config.threshold)
        scheduler.step()
        barrier(ddp)
        ...
        if local_rank == 0:
            ...
            if eval:
                ...
                curr_val_scores = evaluate(model_without_ddp, val_loader, device, config.num_queries, config.threshold, config.crop_size)
