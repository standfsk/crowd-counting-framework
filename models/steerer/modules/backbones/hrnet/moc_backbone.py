...
if torch.__version__.startswith("1"):
    ...
    relu_inplace = False
from ..modules.cnn_blocks import HrnetBasicBlock
from ..modules.cnn_blocks import Bottleneck, LayerNorm
...
class HighResolutionModule(nn.Module):
    def __init__(
        ...
        dp_rates_4module: List[float] = [0.0],
        module_idx: int = 0,
    ) -> None:
...
class HighResolutionNet(nn.Module):
        ...
        **kwargs: Any
        ...
        if os.environ.get("keep_imagenet_head"):
            (
                ...
                self.final_layer,
            ) = self._make_head(pre_stage_channels, bn_type=bn_type, bn_momentum=bn_momentum)
    ...
    def forward(
        self, 
        x: Tensor, 
        which_branch: Optional[int] = None
    ) -> List[Tensor]:
        x = self.stem(x)
        x = self.layer1(x)
    ...
    def init_weights(
        ...
        if pretrained:
            ...
            print('=> loading pretrained model {}'.format(backbone))
...
class MocBackbone(object):
    ...
    def __call__(
        ...
        from .moc_config import MODEL_CONFIGS
        ...
        if arch in [
            ...
            arch_net.init_weights(backbone=arch, pretrained=self.configer.pretrained_backbone)
