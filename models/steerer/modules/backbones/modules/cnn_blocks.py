...
class LayerNorm(nn.Module):
    ...
    """
    def __init__(
        self, normalized_shape: Union[int, List[int]],
        eps: float = 1e-6,
...
class HrnetBasicBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 =nn.Conv2d(
            planes, planes * expansion, kernel_size=3, stride=1, padding=1,bias=False
        )
        self.bn2 = nn.BatchNorm2d(planes * expansion, momentum=BN_MOMENTUM)
...
class ConvxBasicBlock(nn.Module):
    ...
    def __init__(
        ...
        self.pwconv1 = nn.Linear(dim, expansion * dim) # pointwise/1x1 convs, implemented with linear layers
        self.act = nn.GELU()
...
class Bottleneck(nn.Module):
    def __init__(
        ...
        self.conv2 = nn.Conv2d(
            inplanes, planes * expansion, kernel_size=1, bias=False
        )
        self.bn2 = nn.BatchNorm2d(planes* expansion, momentum=BN_MOMENTUM)
        self.conv3 = nn.Conv2d(
            planes * expansion, inplanes, kernel_size=1, bias=False
        )
