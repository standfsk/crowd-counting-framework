...
class one_branch_output(nn.Module):
    def __init__(self, input_channels: int,
                 counter_inchannels: int,
                 tmp_patch_size: int,
                 low_or_high: str = 'medium',
    ...
    def forward(self, current_stage: Tensor,
                last_stage: Optional[Tensor] = None,
                counter: Optional[nn.Module] = None,
                out_branch: bool = False,
                no_distangle: bool = False
        ) -> Tuple[Tensor, Tensor]:
        if no_distangle:
...
class upsample_module(nn.Module):
    ...
    def forward(self,in_list: List[Tensor], counter: nn.Module, counter_copy: nn.Module) -> List[Tensor]:
...
class FusionBYconv(nn.Module):
    ...
    def _make_head(self, pre_stage_channels: List[int]) -> nn.ModuleList:
    ...
    def forward(self, in_list: List[Tensor]) -> Tensor:
