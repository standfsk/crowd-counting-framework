...
class asign_index(torch.autograd.Function):
    @staticmethod
    def forward(ctx: Any, kernel: Tensor, guide_feature: Tensor) -> Tensor:
...
def xcorr_slow(x: Tensor, kernel: Tensor, kwargs: Dict[str, Any]) -> Tensor:
...
def xcorr_fast(x: Tensor, kernel: Tensor, kwargs: Dict[str, Any]) -> Tensor:
...
class Corr(Function):
    @staticmethod
    def symbolic(g: Any, x: Tensor, kernel: Tensor, groups: int) -> Any:
...
class Correlation(nn.Module):
    ...
    def __init__(self, use_slow: Optional[bool] = None) -> None:
    ...
    def forward(self, x: Tensor, kernel: Tensor, **kwargs: Any) -> Tensor:
        if self.training:
            if self.use_slow:
