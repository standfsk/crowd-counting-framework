...
def run(local_rank: int, nprocs: int, config: object) -> None:
    ...
    optimizer, scheduler = get_optimizer(config, model, len(train_loader))
    ...
    model = DDP(nn.SyncBatchNorm.convert_sync_batchnorm(model), device_ids=[local_rank],
                output_device=local_rank, find_unused_parameters=True) if ddp else model
