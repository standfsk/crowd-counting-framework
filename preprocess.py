...
def standardize_dataset_name(dataset: str) -> str:
    if dataset.lower() in ["shanghaitech_a", "sha"]:
        ...
        return "nwpu"
    elif dataset.lower() in ["jhu", "jhu_crowd", "jhu_crowd_v2"]:
        return "jhu"
    else:
        raise ValueError("Select appopriate dataset. [sha, shb, qnrf, nwpu, jhu]")
...
def _jhu(
    ...
    for split in ["train", "val"]:
        ...
        size = len(str(len(image_src_paths)))
        for i, (image_src_path, label_src_path) in tqdm(enumerate(zip(image_src_paths, label_src_paths)), total=len(image_src_paths), desc=split):
